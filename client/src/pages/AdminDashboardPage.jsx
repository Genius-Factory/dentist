...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    let active = true
    setLoading(true)
    Promise.allSettled([getAppointments(getToken), getProfiles(getToken), getUsers(getToken)])
      .then(([appointmentResult, profileResult, userResult]) => {
        if (!active) return
        ...
        const failures = [appointmentResult, profileResult, userResult].filter((result) => result.status === 'rejected').length
        setError(failures ? `Unable to load ${failures === 1 ? 'one dashboard data source' : `${failures} dashboard data sources`}. Please refresh and try again.` : '')
      })
  ...
  const dashboard = useMemo(() => {
    ...
    const activeStaff = users.filter((item) => ['admin', 'superadmin', 'secretary'].includes(normalizeRole(item.role)))
    return { todayAppointments, pending, matching, activeStaff, services: Object.entries(services).sort((a, b) => b[1] - a[1]).slice(0, 4) }
  ...
  const cards = [
    { label: "Today's Appointments", value: dashboard.todayAppointments.length, icon: CalendarDays, note: 'Scheduled for today', tone: 'text-blue-600 bg-blue-50' },
    { label: 'Total Patients', value: profiles.length, icon: Users, note: 'Patient profiles', tone: 'text-indigo-600 bg-indigo-50' },
    { label: 'Active Staff', value: dashboard.activeStaff.length, icon: Stethoscope, note: 'Admin and clinic staff accounts', tone: 'text-violet-600 bg-violet-50' },
  ...
  return <div className="h-[calc(100dvh-4rem)] overflow-y-auto bg-[#f8faff] text-slate-900 md:h-dvh">
      ...
      <main className="mx-auto max-w-[1500px] p-4 sm:p-7">
        ...
        {section === 'admin' && <div className="space-y-5"><ClinicSettingsManager getToken={getToken} /><ServicesManager embedded /><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-bold">User administration</h2><p className="mt-1 text-sm text-slate-500">Manage staff and client roles from the user records page.</p><Link to="/users" className="mt-4 inline-block rounded-xl border border-blue-200 px-4 py-2.5 text-sm font-semibold text-blue-700">Open user records</Link></section></div>}
      </main>
  </div>
}
...
function LoadingTableRow() { return <tr><td colSpan="5" className="px-5 py-10"><span className="flex items-center justify-center gap-2 text-sm text-slate-500"><LoaderCircle className="animate-spin text-blue-600" size={20} />Loading appointments from the database…</span></td></tr> }
